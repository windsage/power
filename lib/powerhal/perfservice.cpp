    ...
    if(cpufreqNeedUpdate || cpufreqHardNeedUpdate) {
        for(int i=0; i >= 0 && i < nCpuNum && i < CPU_CORE_MAX; i++) {
            if(ptCoreTbl[i].byCore == 1) {
                if (sprintf(cpufreqSet_buf, "cpu[%d] min(%s): %d max(%s): %d ",
                ...
                if (sprintf(cpufreqSet_buf, "cluster[%d] min(%s): %d max(%s): %d ",
...
void setCPUFreq(int cluster, int cpu)
{
    ...
    if (gtDrvInfo.proc_powerhal_cpu_ctrl) {
        ...
        if ((freqToSet > hard_freq[cluster].min) && (hard_freq[cluster].min != 0)) {
        ...
        if(sprintf(buf, "%d %d %d ", cpu, freqToSet, maxToSet) < 0) {
            ALOGE("sprintf error");
            return;
        }
        strncat(str, buf, strlen(buf));
            ...
            if (RscCfgTbl[idx].comp == SMALLEST) {
                ...
                if(force_update) {
                    ...
                    for (i = 0; i < SCN_APP_RUN_BASE + nPackNum; i++) {
                        ...
                        RscCfgTbl[idx].canDoOneshotReset == 1 && ptScnList[i].priority == priority &&
    ...
    case DUMP_CPU_CTL:
        ...
        for (i = 0; i < SCN_APP_RUN_BASE+nPackNum; i++) {
            if (ptScnList[i].scn_state == STATE_ON) {
                GetScnName(i, "perfScnDumpAll", scn_name);
                for (j = 0; j < nClusterNum; j++) {
                    ALOGI("%s - cpu:%d", scn_name, ptScnList[i].scn_core_min[j]);
                }
            }
        }
        break;
    ...
    case DUMP_ALL_USER:
        ...
        for (i = nUserScnBase; i < nUserScnBase + REG_SCN_MAX; i++) {
            if (ptScnList[i].pid != -1) {
                ...
                for (j = 0; j < nClusterNum; j++) {
                    if(ptScnList[i].scn_core_min[j] != CPU_CORE_MIN_RESET || ptScnList[i].scn_core_max[j] != CPU_CORE_MAX_RESET) {
                        ALOGI("%s - core[%d]:%d, %d",
                            scn_name, j,
                            ptScnList[i].scn_core_min[j], (ptScnList[i].scn_core_max[j] == CORE_MAX) ? -1 : ptScnList[i].scn_core_max[j]);
                    }
                }

                if(ptScnList[i].scn_gpu_freq != GPU_FREQ_MIN_RESET || ptScnList[i].scn_gpu_freq_max != GPU_FREQ_MAX_RESET ||
                   ptScnList[i].scn_gpu_freq_hard_min != GPU_FREQ_MIN_RESET || ptScnList[i].scn_gpu_freq_hard_max != GPU_FREQ_MAX_RESET ) {
                    ALOGI("%s - gpu freq :%d %d, HL:%d %d",
                        scn_name,
                        ptScnList[i].scn_gpu_freq, ptScnList[i].scn_gpu_freq_max, ptScnList[i].scn_gpu_freq_hard_min, ptScnList[i].scn_gpu_freq_hard_max);
                }
