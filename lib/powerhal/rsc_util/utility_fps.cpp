...
int set_##name##_by_pid(int value, void *scn) { \
    LOG_I("%d", value); \
    return write_cmd_to_file(render_pid, 's', value, PATH_RESCUE_SECOND_ENABLE_BY_PID, #name); \
}

#define FBT_UNSET_ATTR_BY_PID(name) \
int unset_##name##_by_pid(int value, void *scn) { \
    LOG_I("%d", value); \
    return write_cmd_to_file(render_pid, 'u', -1, PATH_RESCUE_SECOND_ENABLE_BY_PID, #name); \
}
...
static void setFstbFps(int fps_high, int fps_low)
{
    ...
    if (fstb_support) {
        if (fps_high == -1 && fps_low == -1) {
            ...
            if(sprintf(fstb_fps, "1 %d-%d", fps_high_now, fps_low_now) < 0)
                return;
...
static void setFstbSoftFps(int fps_high, int fps_low)
{
    ...
    if (fstb_support) {
        if (fps_high == -1 && fps_low == -1) {
            ...
            if(sprintf(fstb_fps, "1 %d-%d", fps_high, fps_low) < 0)
                return;
            LOG_V("set fstb_fps: %s", fstb_fps);
...
int write_string_to_file(int pid, int value, char * path) {
    ...
    if (sprintf(str, "%d %d", pid, value) < 0) {
        LOG_E("sprintf error");
        return -1;
    }
    LOG_I(" %s %s", str, path);
...
int write_cmd_to_file(int pid, char action, int value, char * path, char * cmd) {
    ...
    if (sprintf(str, "%s %c %d %d", cmd, action, pid, value) < 0) {
        LOG_E("sprintf error");
        return -1;
    }
    LOG_I(" %s %s", str, path);
...
int set_vsync_app_fps_disable_by_pid(int value, void *scn) {
    LOG_I("%d", value);
    return write_string_to_file(render_pid, value, PATH_FSTB_VSYNC_FPS_DISABLE_BY_PID);
}

int unset_vsync_app_fps_disable_by_pid(int value, void *scn) {
    return write_string_to_file(render_pid, 0, PATH_FSTB_VSYNC_FPS_DISABLE_BY_PID);
}
...
FBT_SET_ATTR_BY_PID(RT_prio1)
FBT_UNSET_ATTR_BY_PID(RT_prio1)

FBT_SET_ATTR_BY_PID(RT_prio2)
FBT_UNSET_ATTR_BY_PID(RT_prio2)

FBT_SET_ATTR_BY_PID(RT_prio3)
FBT_UNSET_ATTR_BY_PID(RT_prio3)
...
FBT_SET_ATTR_BY_PID(engine_cooler_enable)
FBT_UNSET_ATTR_BY_PID(engine_cooler_enable)
