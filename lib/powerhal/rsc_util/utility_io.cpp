...
#define UFS_PATH_LEN            (100)

static char ufs_path[UFS_PATH_LEN] = {0};
...
static void setup_ufs_path(void)
{
    ...
    while ((de = readdir(d)) != 0) {
        if (!strncmp("ufshci", de->d_name, 6)) {
            if (sscanf(de->d_name, "ufshci@%s", base_addr) < 1) {
                LOG_E("sscanf error");
                break;
            }
            if (sprintf(ufs_path, "/sys/devices/platform/soc/%s.ufshci/", base_addr) < 0)
                LOG_E("sprintf error");
            break;
        }
    }
...
static void get_ufs_setting(const char *name, char *value, size_t len)
{
    struct stat stat_buf;
    char path[UFS_PATH_LEN];

    if (!name || !value) {
        LOG_E("%s: name or value invalid", __func__);
        return;
    }

    setup_ufs_path();

    if (strlen(ufs_path) + strlen(name) > UFS_PATH_LEN) {
        LOG_E("%s: path length overflow", __func__);
        return;
    }

    strcpy(path, ufs_path);
    strcat(path, name);
...
{
    get_ufs_setting("auto_hibern8", ufs_auto_hibern8,
...
{
    get_ufs_setting("irq/smp_affinity", ufs_irq_affinity,
...
int set_ufs_auto_hibern8(int value, void *scn)
{
    ...
    if (!strlen(ufs_path)) {
        LOG_E("%s: no ufs path", __func__);
        return -1;
    } else if (strlen(ufs_path) + strlen("auto_hibern8") > UFS_PATH_LEN) {
        LOG_E("%s: path length overflow", __func__);
        return -1;
    }

    strcpy(path, ufs_path);
    strcat(path, "auto_hibern8");

    if (!value)
        set_value(path, ufs_auto_hibern8);
    else
...
int set_ufs_irq_affinity(int value, void *scn)
{
    ...
    if (!strlen(ufs_path)) {
        LOG_E("%s: no ufs path", __func__);
        return -1;
    } else if (strlen(ufs_path) + strlen("irq/smp_affinity") > UFS_PATH_LEN) {
        LOG_E("%s: path length overflow", __func__);
        return -1;
    }

    strcpy(path, ufs_path);
    strcat(path, "irq/smp_affinity");

    if (!value) {
        set_value(path, ufs_irq_affinity);
    } else {
...
{
    char dbg_tp_unregister[UFS_PATH_LEN];
    char skip_btag[UFS_PATH_LEN];

    if (!strlen(ufs_path)) {
        LOG_E("%s: no ufs path", __func__);
        return -1;
    } else if (strlen(ufs_path) + strlen("dbg_tp_unregister") > UFS_PATH_LEN) {
        LOG_E("%s: path length overflow", __func__);
        return -1;
    } else if (strlen(ufs_path) + strlen("skip_blocktag") > UFS_PATH_LEN) {
        LOG_E("%s: path length overflow", __func__);
        return -1;
    }

    strcpy(dbg_tp_unregister, ufs_path);
    strcat(dbg_tp_unregister, "dbg_tp_unregister");
    strcpy(skip_btag, ufs_path);
    strcat(skip_btag, "skip_blocktag");

    switch (value) {
    case 0:
        set_value(dbg_tp_unregister, 0);
        set_value(skip_btag, 0);
        break;

    case 1:
        set_value(dbg_tp_unregister, 1);
        break;

    case 2:
        set_value(skip_btag, 1);
        break;
