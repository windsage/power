...
int loadConTable(const char *file_name)
{
    ...
    while (elmtCMD) {
        ...
        ALOGD("[loadConTable] cmd:%s, path:%s, normal:%d, default:%d, init_set_default:%d, prirority: %d", tConTable[idx].cmdName.c_str(),
            tConTable[idx].entry.c_str(), tConTable[idx].normalVal, tConTable[idx].defaultVal, tConTable[idx].init_set_default, tConTable[idx].priority);
