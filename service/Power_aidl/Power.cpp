...
ndk::ScopedAStatus Power::setMode(Mode type, bool enabled) {
    ...
    /* USD: add screen off hint by chuan.gong at 20250103 start */
    case Mode::DISPLAY_INACTIVE:
        trigger_lock = 1;
        hint = DISPLAY_INACTIVE; // mtkpower hint ID in mtkpower_hint.h
        if(enabled > 0) {
            dur = 0;  // 0 means infinite duration
        } else {
            dur = -1; // -1 means disable
        }
        LOG_I("Mode::DISPLAY_INACTIVE-2: %d, %d", enabled, dur);
        break;
     /* USD: add screen off hint by chuan.gong at 20250103 end */
   default:
         LOG_E("unknown type");
         break;
...
ndk::ScopedAStatus Power::isModeSupported(Mode type, bool* _aidl_return) {
    if (type >= Mode::DOUBLE_TAP_TO_WAKE && type <= Mode::GAME_LOADING) {
...
ndk::ScopedAStatus Power::isBoostSupported(Boost type, bool* _aidl_return) {
    if (type >= Boost::INTERACTION && type <= Boost::CAMERA_SHOT) {
        LOG_I("supported:%d", type);
        *_aidl_return = true;
    } else {
        LOG_I("unsupported:%d", type);
...
ndk::ScopedAStatus Power::getHintSessionPreferredRate(int64_t* outNanoseconds) {
    ...
    *outNanoseconds = 1000000000/devicePreferredRate;
    LOG_I("outNanoseconds = %ld", *outNanoseconds);
...
ndk::ScopedAStatus Power::createHintSession(int32_t tgid, int32_t uid, const std::vector<int32_t>& threadIds, int64_t durationNanos, std::shared_ptr<IPowerHintSession>* _aidl_return) {
    ...
    if (threadIds.size() == 0) {
        LOG_I("Error: threadIds.size() shouldn't be %d", threadIds.size());

        *_aidl_return = nullptr;
        return ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_ARGUMENT);
    }

    LOG_I("TGID=%d, UID=%d, targetDurationNanos=%ld", tgid, uid, durationNanos);
    ...
    std::shared_ptr<IPowerHintSession> powerHintSession = ndk::SharedRefBase::make<PowerHintSession>(sessionId, tgid, uid, threadIds, durationNanos);
