...
ScopedAStatus PowerHintSession::updateTargetWorkDuration(int64_t targetDurationNanos) {
    ALOGI("[ADPF_updateTargetWorkDuration] targetDurationNanos = %ld", targetDurationNanos);
    _ADPF_PACKAGE msg;
...
ScopedAStatus PowerHintSession::reportActualWorkDuration(const std::vector<WorkDuration>& durations) {
    for (int i = 0; i < durations.size(); i ++) {
        ALOGD("[ADPF_reportActualWorkDuration] durations[%d].timeStamp=%ld, durations[%d].workPeriodStartTimestamp=%ld, \\
            durations[%d].duration=%ld, durations[%d].cpuDuration=%ld, durations[%d].gpuDuration=%ld",
              i, durations[i].timeStampNanos,
              i, durations[i].workPeriodStartTimestampNanos,
              i, durations[i].durationNanos,
              i, durations[i].cpuDurationNanos,
              i, durations[i].gpuDurationNanos);
    }
...
ScopedAStatus PowerHintSession::pause() {
    ...
    ALOGI("[ADPF_pause] sid: %d", this->id);
...
ScopedAStatus PowerHintSession::resume() {
    ...
    ALOGI("[ADPF_resume] sid: %d", this->id);
...
ScopedAStatus PowerHintSession::close() {
    ...
    ALOGI("[ADPF_close] sid: %d", this->id);
...
ScopedAStatus PowerHintSession::sendHint(SessionHint hint) {
    ALOGI("[ADPF_sendHint] hint:%d", hint);
    _ADPF_PACKAGE msg;
...
ScopedAStatus PowerHintSession::setMode(SessionMode mode, bool enabled) {
    ALOGI("[ADPF_setMode] mode:%d, enabled:%d", mode, enabled);
...
ScopedAStatus PowerHintSession::getSessionConfig(SessionConfig* _aidl_return) {
    ALOGI("[ADPF_getSessionConfig] sid: %d", this->id);
